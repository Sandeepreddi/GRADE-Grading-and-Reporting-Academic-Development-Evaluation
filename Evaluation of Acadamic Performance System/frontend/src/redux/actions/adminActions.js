...
export const deleteNotice = (noticeId) => async (dispatch) => {
  try {
    // Assuming you need to send the formData in the request body or as query parameters
    ...
    console.error("Delete Notice Error:", error); // Log error for debugging

    dispatch({
      type: SET_ERRORS,
      payload: error.response?.data || { backendError: "Something went wrong!" },
    });
  }
};
